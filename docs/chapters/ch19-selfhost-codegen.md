...
> **この章のゴール**
> `selfhost/codegen.my` が、C 版と**まったく同じ LLVM IR** を出す。
> **そして、その IR が本当に動く。**
>
> ```bash
> $ diff <(./build/mythonc -S t.my) <(./build/stage1-codegen t.my) && echo 一致
> 一致
> $ ./build/stage1-codegen t.my > t.ll
...
Mython 版は `list[str]` に溜めて、最後に `join` します。
...
```python
# lib/io.my
extern def my_print_raw(s: str) -> None
...
```bash
$ ./build/stage1-codegen selfhost/emit_ir.my | wc -l
27216
$ diff <(./build/mythonc -S selfhost/emit_ir.my) \
       <(./build/stage1-codegen selfhost/emit_ir.my) && echo 一致
一致
```

**stage1（Mython 製コンパイラ）が、stage1 自身の全ソース（約 5,700 行）から
...
**② 値を返さない式の扱い。**
C 版は `NULL` を返しますが、Mython の `str` に None は使えません
...
✅ lib/io.my に print_raw（言語には足していない）
```
...
| ファイル | 変更 |
|---|---|
| `selfhost/codegen.my` | **新規 1180 行** |
| `selfhost/emit_ir.my` | **新規 60 行** |
| `lib/io.my` / `runtime` | `print_raw` |
...
| **`target triple` を環境変数で渡す** | ビルド時に埋め込めない（プリプロセッサが無い）。第18章の `MYTHON_LIB_DIR` と同じ手 |
...
```bash
git add -A
git commit -m "第19章: Mython でコード生成器を書く"
```
...
1. `codegen.my` の `gen_if` で番号を「使うたびに取る」形に変えて、
...
5. **`selfhost/emit_ir.my` の IR（27,216 行）を眺めて**、
   自分が書いた Mython のコードがどんな IR になっているか確かめる
