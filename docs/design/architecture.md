...
> `mythonc`（C 言語製、stage0 コンパイラ）の内部構造です。
...
今回は**最終的に Mython でこれを書き直す**ことが目的です。
...
```c
// 1. .ll を一時ファイルに書き出す
char *ll_path = "/tmp/mythonc-XXXXXX.ll";
...
```
mythonc internal error: src/codegen.c:412: 到達しないはずのコード
...
| 種類 | 方法 | 例 |
|---|---|---|
| **終了コードテスト** | `.my` をコンパイル・実行し、終了コードを検証 | `42` → 42 |
...
`tests/cases/*.my` の先頭コメントに期待値を書く方式にします。
...
```bash
make test          # 全テスト
make test-one CASE=tests/cases/prec.my
```
...
| ch16-19 | `selfhost/*.my` が `src/*.c` と並行して育つ |
